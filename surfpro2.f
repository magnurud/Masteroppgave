...
c     beta adjusts the limit of the working array.
     ...
     &     err_new
      ...
      real wrksurf(nsurf,3),wrkbdry(nwork,3)
       ...
       call getfaceinfo(midpoint,radius,ie,f)

       ! Finding the points of interest
       call 
     & createworkingarray(wrkbdry,wrksurf,midpoint,iter,beta,radius)
...
c     rad      - radius of the current element
     ...
     &      + sn(3)*(z-wrkbdry(itemp,3))/rtemp**m
      ...
      if(k/d/(2*rad)*lx1.le.0.5) then !restrict rel. movement
...
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
      subroutine createworkingarray(wrkbdry,wrksurf,mid,iter,beta,rad)
