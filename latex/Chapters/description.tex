...
\begin{align}
    ...
    \nu_{sgs} &= C_dl^2\sqrt{2s^r_{ij}s^r_{ij}}.
    \label{eq:boussinesq}
\end{align}
%
...
Let $a,b$ denote two distinct filters with corresponding sub-grid scale tensors 
$\tau^a_{ij}$ and $\tau^b_{ij}$ and filter widths $l_a,l_b$. 
By defining $a_{ij}=l_a^2\mathcal{S}^a = l_a^2\sqrt{2s^{a}_{ij}s^{a}_{ij}}$ the subgrid scale tensor can 
be written as $\tau^a_{ij} = 2C_da_{ij} $. Applying both filters on the N-S equation 
results in a sub-grid scale tensor defined as $L_{ij} = (u_iu_j)^{ab}-u_i^{ab}u_j^{ab}$.
By the Germano identity this tensor can be written as 
\begin{align}
    L_{ij} = (\tau_{ij}^a)^b - \tau_{ij}^b(u_i^a,u_j^a).
    \label{eq:germanoid}
\end{align}
(OBS!! $\tau_{ij}^b(u_i^a,u_j^a)= \tau_{ij}^b(u_i,u_j)$ )!!! 
By working with this expression assuming a dynamic constant uneffected by the filter 
one obtains
\begin{align}
    \begin{split}
 L_{ij} &= 2((C_da_{ij})^b-C_db_{ij}),\\
        &\approx 2C_d(a_{ij}^b-b_{ij}),\\
        &= 2C_dl_c^2 M_{ij}.
    \end{split} \\
        M_{ij} &= (\mathcal{S}^as_{ij}^a)^b - (l_a/l_c)^{2}\mathcal{S}^bs_{ij}^b\\
        l_c &= l_a/l_b\\
        C_s &= \frac{L_{ij}}{2l_c^2M_{ij}}
    \label{eq:dynsmagderivation}
\end{align}

\colorbox{yellow}{fix notation in this section!!! }
