...
%\item  $\nabla \cdot \tau$ 
    ...
    - The Reynolds number defined as $UL/\nu$ where $U$ is the velocity scale, $L$ is the length scale and $\nu$ 
      ...
      and the viscous length scales. Notice that for large Reynolds numbers the unstable non-linear term 
      dominates the transportation. Turbulent flows are characterized by a high Reynolds number.
    ...
    of the Reynolds stress tensor that can be made under the assumption of incompressibility. The more general formulation that 
...
$\mathbf{n}$ and $\mathbf{t}$ will denote the normal vector and the tangential surface to the boundary surface.
...
\end{align}
%
The contributions from the boundary as a result of the application of the divergence theorem is included in the 
force term.
...
using a set of basis functions. The idea is that the solution $(\mathbf{u},p)$ is approximated 
...
The N-S equations explains how a fluid will behave, and solving these provides a pressure-velocity field on the 
domain of interest. 
...
The filter is assumed to commutes with 
...
assumption an computable expression is developed for the dynamical constant.
...
of the dynamic Smagorinsky constant. Remember that $a$ is the filter corresponding tu the numerical discretization, while 
