...
\begin{align}
    \begin{split}
        v^* &\leftarrow \frac{dv}{dt}=Nv,\\
    p^{n+1} &\leftarrow \Delta\: p = \nabla \cdot \frac{v^*}{\Delta t},\\
    v^{**} &\leftarrow  \frac{dv^*}{dt} =Dp^{n+1},\\
    v^{n+1} &\leftarrow \frac{dv^{**}}{dt}= Lv^{**}.
    \end{split}
    \label{eq:fracstep}
\end{align}
...
\begin{align}
    \begin{split}
    (v^{n+1}-v^{n})/\Delta t  &= Lv^{n+1},\\
    -Lv^{n+1}+\frac{1}{\Delta t} v^{n+1} &= v^{n}.
    \end{split}
    \label{eq:fracHelm}
\end{align}
Knowing that $L$ is the discrete laplacian operator and $v^n$ is a known variable this is 
