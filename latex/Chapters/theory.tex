...
\begin{align}
    ...
    \label{eq:Legendre}
\end{align}
With equations \ref{eq:Legendre} and \ref{eq:GLL-nodes} the basis functions $\phi_j$ can be stated as 
\begin{align}
...
$\{x_i\}$ beeing the solutions to \ref{eq:GLL-nodes}. Note that $\phi_j(x_i) = 0 \text{ when } i \neq j$,
the expansion coefficients in \ref{eq:spectralprojection} are then chosen as $a_i = u(x_i)$ to minimize the projection error in $L^2(\Omega)$. 
