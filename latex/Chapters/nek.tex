...
they apply RANS, LES, DNS or something else. Allthough most solvers offer multiple of the settings
...
guaranteeing great speedup. All the parallelization is accesed through subroutines
...
the meshroutines are further elaborated an overview of the file-structure will be presented.
...
\end{lstlisting}
\endgroup
This file follows the standar Fortra77 syntax and parameters can be added as the user likes. 
...
the variable. When ending with $v$ or 1, the variable is connencted to the velocity. 
...
that are not used until the initialization of the case. The strucuture of the file is given in table~\ref{tab:reafile}.
...
\colorbox{yellow}{How does a preconditioner work in Nek ?}
