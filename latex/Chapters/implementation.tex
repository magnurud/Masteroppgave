...
necessary tools in order to improve the functionality. 
...
$0.0013$s for a total of $28$s. The right plot in figure~\ref{fig:inflow} is an instantanous 
picture of the velocity in x-direction, notice how the pattern repeats itself along the 
y-axis. This is because the inflow data was generated in a smaller channel, $\approx 1/3$ of the 
width of the computational domain used for the data sampling. 
An interpolation algorithm had to be implemented in order to adjust the inflow-data to the 
computational mesh, this was done directly in the \verb|.usr| file. 
\begin{figure}
\centering
  \centerline{
...
which is done by solving the N-S equations until a steady flow is reached. This implies that the
spatial accuracy will be essential and would imply that the solution provided
by a SEM solver such as Nek5000 should be very good compared to other FV-based solvers.
%
\begin{table}[h]
    \centering
    \begin{tabular}{c l l}
...
An interesting test that is performed on this case is how different settings in Nek5000 will affect the estimation of the drag and lift coefficients.
...
The Gordon Hall algorithm which is described in~\ref{GH} was already implemented as a function in the Nek library.
...
The curved edges are specified in the \verb|.rea| file and have until now been read as a second degree polynomial or as a part of a 
...
The routine was made as automatic as possible, and can be summed up in these simple steps
\begin{enumerate}
...
\colorbox{red}{lack a verification case}
