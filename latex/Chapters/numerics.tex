...
Keeping $h$ constant and increasing $p$ will provide esponential decay of the error, but the sparsity will be reduced and all integrals solved will require 
...
\begin{align}
    ...
    = \sum_n \rho_nf \Psi_m(\mathbf{x}_{n}),\qquad \text{for } m = 1,\cdots,N^2,
    \label{eq:Helmholtzquad}
\end{align}
%
where $\rho_n$ is the quadrature weight for the $n$th node, and $\mathbf{x}_n$ is the vector containing the coordinates to the $n$th node.
