...
is identical if $\alpha_i = \lambda_2\Delta t/(1+\lambda_2\Delta t)\approx \lambda_2\Delta t$. 
...
$M^{-1}C(\mathbf{u})$, which is known to scale as 
$\mathcal{O}(N^{2})=\mathcal{O}(\Delta x_{\min}^{-1})$, and be proportional to $\mathbf{u}$.
The following inequality has to be satisfied for the scheme to be stable, 
%
\begin{align}
