...
steps the solution $(\mathbf{u},p)$ is deveoped one time-step
...
\begin{align}
    \begin{split}
        ...
        +\Delta t (C\mathbf{u}^{n+1}+M\mathbf{f}^{n+1}),\\
        \Delta p^{n+1} &= \nabla \cdot \left(  \frac{\mathbf{u}^{*}}{\Delta t}\right),\\
        \mathbf{u}^{**} &= \mathbf{u}^{*} + \Delta t D^T p^{n+1},\\
        b_0\mathbf{u}^{n+1} &= \mathbf{u}^{**} -\Delta t A\mathbf{u}^{n+1}.
    \end{split}
    \label{eq:fracstep}
\end{align}
