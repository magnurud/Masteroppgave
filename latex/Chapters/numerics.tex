...
%
\begin{align}
    \inf_{q\in Q}\sup_{\mathbf{v}\in V}\frac{\mathcal{B}(\mathbf{v},q)}{||\mathbf{v}||_V||q||_Q} \ge b,
    \label{eq:infsup}
\end{align}
%
...
For a Spectral Element formulation of this problem a valid choice of subspaces $(V,Q)$
is $\left(  \left[  P_N\cap H^1_0\right]^3,P_{N-2} \cap L^{2}_{0}\right)$. This will be referred to as the 
...
polynomial degree for pressure and velocity. The subspaces $(V,Q)$ is therefore chosen as 
$\left( \left[ P_N\cap H^1_0 \right]^3,P_{N} \cap L^{2}_{0} \right)$,
...
other areas. It offers a precise mathematical foundation and due to the local support of the 
basis functions it guaranties a sparse system. The decomposition of the geometrical domain into a finite amount of elements,
...
$\{\xi_i\}$ being the solutions to \eref{eq:GLL-nodes}. Note that $\psi_j(\xi_i) = \delta_{ij}$.
The expansion coefficients in \eref{eq:spectralprojection} are then chosen as $a_i = u_i :=u(\xi_i)$. 
...
The method outlined in this section is similarly to FEM also a projection method, 
...
\begin{align}
    ...
    (\mathbf{J}_k^{-T}\hat{\nabla} \hat{\psi}_j) J_k d \hat{\Omega} + 
    u_i\lambda \int_{\hat{\Omega}_k} \hat{\psi}_i \hat{\psi}_j J_kd \hat{\Omega} \right)
    &= \int_{\hat{\Omega}_k}\hat{f} \psi_j J_k d \hat{\Omega} \qquad \forall \psi_j \in V_h.  \\
    \sum_i\left(  u_i\int_{\hat{\Omega}_k}\hat{\nabla}^T \hat{\psi}_i\mathbf{J}_k^{-1}
    \mathbf{J}_k^{-T}\hat{\nabla} \hat{\psi}_j J_k d \hat{\Omega} + 
    u_i\lambda \int_{\hat{\Omega}_k} \hat{\psi}_i \hat{\psi}_j J_k d \hat{\Omega} \right)
    &= \int_{\hat{\Omega}_k}\hat{f} \psi_j J_k d \hat{\Omega} \qquad \forall \psi_j \in V_h.
    \end{split}
...
to \thref{thm:femconvergence} and~\ref{thm:smconvergence}.
...
\begin{theorem}
    Let $\Omega = [-1,1]$ and $\left\{ \Omega_k \right\}$ be the set of non-overlapping elements. 
...
\end{theorem}
%
It should be mentioned that the result in~\thref{thm:smconvergence} holds for SEM as well.
...
Let $u = \sum_{i=0}^{N} \hat{u}_i L_i$ be some function, where $L_i$ denote the Lagrange
...
from the highest mode $N$ to the mode $N-2$. The rest of the coefficients remain unchanged.
...
The filter is proved to be an effective stabilization method and it preserves the 
...
A good physical description of the filter has not yet been presented, this 
...
as the heat equation and is known to diffuse or smooth out the initial condition. 
...
\end{align}
%
This relation is derived in Appendix~\ref{AppendixA}.
...
Doing a first order implicit Euler on \eref{eq:ivps2} can be written as 
%
\begin{align}
    \begin{split}
...
To illustrate this effect~\fref{fig:entries} shows the non-zero entries of the
...
Using the known formula of the norm from~\eref{eq:relations} we can write this as 
\begin{align}
...
imposing $A_{ij}=0$ for $j-i>2$, then the Laplacian and the filtering would have the same non-zero entries 
and by choosing $\alpha_i$ as indicated in~\eref{eq:alpha2} would yield equality. Notice in particular the similarity of 
...
%\colorbox{red}{omit this section}
%Before ending this chapter let us give some estimates for the coefficients $\lambda_1,\lambda_2$ in~\eref{eq:ivps}.
%For the simulations presented in~\cref{case1}, the polynomial degree was $N = 7$ and 
%the relation between filtering of the last node and the time step was $\alpha_0/\Delta t = 50$.
%Solving~\eref{eq:alpha1} and $\ref{eq:alpha2}$ for $\lambda_1$ and
%$\lambda_2$ yields for the last Legendre mode
%\begin{align}
    %\lambda_1 = 50/66, \qquad \lambda_2 = 50.
    %\label{eq:coeffs}
%\end{align}
%This implies that when the filter is applied in the simulations performed in~\cref{case1} it corresponds to 
%solving for the highest mode the PDEs 
%\begin{align}
    %\begin{split}
    %\frac{\partial v }{\partial s} &=  \frac{50}{60}\Delta v, \qquad v(0) = u^n, \qquad 0<s<\Delta t, \\
    %\frac{\partial u }{\partial s} &= -50u, \qquad u(0) = u^n, \qquad 0<s<\Delta t.
    %\end{split}
    %\label{eq:ivpfinal}
%\end{align}
...
Applying a quadrature rule of a not sufficiently high order results in an 
amplification of the lower modes, attempting to compensate for the higher order modes omitted. 
...
a reference domain $\hat{\Omega} = [-1,1]^d$ for efficiency and implementation purposes. The Gordon-Hall 
...
\end{align}
$f$ is here a function of $u$ and $t$, while $g$ is only dependent of the time $t$. Let superscript denote the time step, 
such that $g^{n}=g(n\Delta t)$ for some fixed time step $\Delta t$. One step applying a $k$th order
...
on the scheme. When applied on the N-S equations it is the convection term $\Delta t M^{-1}C(\mathbf{u})\mathbf{u}$
as observed in~\eref{eq:fracstep} that is evaluated by an explicit scheme and it is these eigenvalues that 
needs to be constrained. Let $h^e$ and $u^e$ denote the grid size and the maximum absolute velocity in element $e$.
...
Throughout this section the NS-equation will be considered in its operational form as introduced in~\eref{eq:NSMatrixform}
%
\begin{align}
    M\frac{d \mathbf{u}}{dt} + C(\mathbf{u})\mathbf{u} = -A\mathbf{u} +D^T p +M\mathbf{f}, \qquad D\mathbf{u} = 0
    \label{eq:NSoperator}
\end{align}
%
...
%
\begin{align}
    \frac{dQ(t)M\mathbf{u}}{dt} &=  Q(t)M\frac{d\mathbf{u}}{dt} + \frac{d}{dt}\left[  Q(t)M\right]\mathbf{u},\\
    &= Q(t)M\frac{d\mathbf{u}}{dt} + Q(t)C(\mathbf{u})\mathbf{u}. 
    \label{eq:integrationalfactor}
\end{align}
%
This way \eref{eq:NSoperator} can be written as 
\begin{align}
    \frac{d Q(t)M\mathbf{u}}{dt} =Q(t)( -A\mathbf{u} +D^T p +M\mathbf{f}).
    \label{eq:NSoperatorOIFS}
\end{align}
Evaluating this equation with a BDFk-scheme results in a system 
\begin{align}
    \sum_{j=0}^{k}b_jQ(t^{n+1-j})M\mathbf{u}^{n+1-j} =\Delta t \: Q(t^{n+1})( -A\mathbf{u}^{n+1} +D^T p^{n+1} +M\mathbf{f}^{n+1}).
    \label{eq:NSOIFS1}
\end{align}
Applying the fact that $Q(t^{n+1}) = I$ enables \eref{eq:NSOIFS1} to be written as 
\begin{align}
    b_0M\mathbf{u}^{n+1} + \sum_{j=1}^{k}b_jQ(t^{n+1-j})M\mathbf{u}^{n+1-j} 
    =\Delta t ( -A\mathbf{u}^{n+1} +D^T p^{n+1} +M\mathbf{f}^{n+1}).
    \label{eq:NSOIFS1}
\end{align}
...
First of all the auxiliary variable $\tilde{\mathbf{u}}_j$ is defined such that $Q(t^{n+1-j})M\mathbf{u}^{n+1-j} = M\tilde{\mathbf{u}}_j$
thus enabling the summation expression to be found by solving the initial value problem 
\begin{align}
    \begin{split}
    M\frac{d\tilde{\mathbf{u}}_j}{ds} &= -C(\tilde{\mathbf{u}}_j(s))\tilde{\mathbf{u}}_j(s) , \qquad t^{n+1-j}\leq s\leq t^{n+1}\\
    \tilde{\mathbf{u}}_j(t^{n+1-j}) &= \mathbf{u}(t^{n+1-j}).
    \end{split}
    \label{eq:IVP}
\end{align}
...
solving \eref{eq:NSOIFS1} and an explicit multistage scheme for solving \eref{eq:IVP}. When applied in this thesis the 
...
Solving~\eref{eq:IVP} with a multi-stage method implies a bit more work per time step, but as it stabilizes the routine larger advances in 
...
divergence-free condition to obtain the second equation in~\eref{eq:nonstandardstokes}.
The vector identity 
$ -\Delta \mathbf{u}=-\nabla\nabla\cdot\mathbf{u}+\nabla\times\nabla\times\cdot\mathbf{u}$ 
combined with the divergence-free condition yields the boundary condition for the pressure.

Fractional step exploits this algebraic splitting and the algorithm can be divided into
...
an OIFS procedure to gain stability. This implies using the discretization introduced in \eref{eq:NSOIFS1}
...
This method provides an efficient algorithm, and by subtracting $1/\Delta tM^{-1}D^Tp^n$ 
from the first equation in~\ref{eq:fracstep} the Poisson equation can be solved for 
$\delta p^{n+1}=p^{n+1}-p^{n}$ instead of $p^{n+1}$.
This yields a scheme with errors in the $L_2$-norm of orders $\mathcal{O}(\Delta t)$ for
...
\begin{pmatrix}
    ...
    \label{eq:Matform}
\end{equation}
The splitting that will be done in the next step will use the 
pressure difference $\delta p^{n+1} = p^{n+1}-p^n$ as was done for the fractional step algorithm.
\eref{eq:Matform} can be restated as 
\begin{equation}
\begin{pmatrix}
    H & -D^T \\ 
    D & 0
...
Solving this exactly is known as the Uzawa algorithm and is
...
To clarify what is going on, a brief description of each step is given,
...
The second step in \eref{eq:PCstep1} is the discrete Poisson equation for the pressure and will 
