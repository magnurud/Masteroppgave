...
In the previous chapter the N-S equation was presented with a weak formulation and several solution algorithms. 
The choice of algorithms and solution spaces required a more thourough analysis which is normally performed on the 
Stokes problem. The (steady?) Stokes problem does not include the convection term but the highest order terms are all present
and is therefore a valid equation to perform this necessary analysis \cite{Karniadakis}. The stokes problem is given 
as 
%
\begin{align}
    \begin{split}
...
\end{align}
%
$\beta$ is here some positive constant. For a spectral element formulation of this problem which will be elaborated in chapter
~\ref{sem}, a common choice of subspaces $(X,Q)$ is $(P_N\cap H^1_0,P_{N-2} \cap L^{2}_{0})$. This will be referred to as the 
$P_NP_{N-2}$-formulation where $P_N$ denotes the space of polynomials up to degree $N$.
It was however proved by Guermond in~\cite{Guermond} that the fractional step algorithm requires a 
