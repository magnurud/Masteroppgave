...
This filter in~\ref{eq:filtereffect} is often applied to the $k$ highest Legendre polynomials,
...
vector containing the the Legendre coefficients.
\begin{align}
...
subsection is an attempt to show some resemblence between known properties 
of some differential equations and the mathematical filter described in the previous section.

Let $u$ be some smooth continuos function,$\bar u = [u_1L_1(x),u_2L_2(x), \cdots]$
is the corresponding legendre basis with 
appropriate coefficients
and $P_{N-k}$ is the diagonal matrix with ones on the $k$ last entries and $0$ on the rest.
...
\begin{align}
    ...
    &= \Delta P_{N-k}\bar v^T , \qquad v(0) = v_0, \\
        \frac{\partial \bar u }{\partial t} 
        &=   -P_{N-k}\bar u^T , \qquad u(0) = u_0.
    \label{eq:ivps2}
    \end{split}
\end{align}
...
\end{align}
The matrix-matrix product $AP_{N-k}$ will lead to a laplacian matrix where the $N-k$ are zero
while the last $k$ remains unchanged. This modified laplacian will be denoted $A^*$.
Omitting the initial conditions, and doing a first order explicit 
time-integration of~\ref{eq:ivps2} can be written as 
\begin{align}
    u^{n+1} &= u^n + \Delta t A^* u^n = (I + \Delta t A^*)u^n, \\
    u^{n+1} &= u^n - \Delta t P_{N-k}u^n = (I - \Delta t I_k)u^n.
    \label{eq:timestep}
\end{align}
Notice the resemblence to the application of the filter in~\ref{eq:filterstepwise}. The second step
is identical if $\alpha_i = \Delta t$. The truncated laplacian $A^*$ is not equal the corresponding
filter matrix $\mathcal{F}_{\alpha}$, but there are some similarities between these to matrices.
They are both upper triangular with zeros along the diagonal and non-zero entries on the second 
...
if $A_{ij}=0$ if $i-j>2$, then the laplacian and the filtering would have the same structure 
and a clever choice of $\alpha_i$ would yield equality. 
